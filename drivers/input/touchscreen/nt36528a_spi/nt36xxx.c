...
char nvt_tp_fw_name[24] = {0};
char nvt_tp_mp_name[24] = {0};
...
static int32_t get_lockdown_info_by_lcd(void)
{
    ...
    char lockdown_info[64];
    lcm_name = of_find_node_by_path("/chosen");
    if (lcm_name) {
        videolfb_tag = (struct tag_videolfb *)of_get_property(lcm_name,"bootargs",(int *)&size);
        if (!videolfb_tag) {
            NVT_ERR("Invalid lcm lockdown info\n");
            return -1;
        }
        temp1 = strstr(videolfb_tag->lcmname,"dsi_display_lockdown=");
        if(temp1 != NULL) {
            temp2 = strstr(temp1,"=");
            strncpy(lockdown_info, temp2+1, 16);
        } else {
            strcpy(lockdown_info, "Unknown lockdown info");
        }
    }
    snprintf(lockdown_info, 64 , "%s\n", lockdown_info);
    update_lct_tp_lockdown_info(lockdown_info);
    NVT_LOG("read lockdown_info = %s\n", lockdown_info);
    return 0;
}
...
static void nvt_ts_usb_plugin_handler(int usbFlag)
{
    uint8_t buf[8] = {0};
    int32_t ret = 0;
    if(!ts->sensor_tp_fw_status){
        NVT_ERR("TP's firmware is not raedy\n");
        return;
    }
    if ( !bTouchIsAwake && !proximity_event) {
        NVT_ERR("TP is already suspend\n");
        return;
    }
    mutex_lock(&ts->lock);
    msleep(35);
...
static int nvt_power_usb_notifier_callback(struct notifier_block *nb, unsigned long ev, void *v)
{
    ...
    queue_work(ts->event_wq, &ts->online_work);
    }
    return 0;
}
#endif
...
info_retry:
    ...
    if (0 == strcmp(nvt_tp_fw_name, BOOT_UPDATE_FIRMWARE_NAME_XINLI)) {
        NVT_LOG("tp_info: [Vendor]xinli,[FW]0x%02x,[IC]nt36528a\n",ts->fw_ver);
        sprintf(tp_info_buf, "[Vendor]xinli,[FW]0x%02x,[IC]nt36528\n", ts->fw_ver);
    } else if(0 == strcmp(nvt_tp_fw_name, BOOT_UPDATE_FIRMWARE_NAME_TXD)) {
        NVT_LOG("tp_info: [Vendor]TXD,[FW]0x%02x,[IC]nt36528a\n",ts->fw_ver);
        sprintf(tp_info_buf, "[Vendor]TXD,[FW]0x%02x,[IC]nt36528\n", ts->fw_ver);
    } else {
...
#ifdef FACTORY_BUILD
            set_lct_tp_proximity_data(3);
            NVT_LOG("factory build get proximity on event.\n");
#else
            set_lct_tp_proximity_data(proximity_level);
#endif
            NVT_LOG("get proximity on event, level=%d.\n", proximity_level);
        } else if (proximity_state == PROXIMITY_OFF) {
            set_lct_tp_proximity_data(0);
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
    ...
    ret = init_lct_tp_proximity_data();
    if (ret < 0) {
        NVT_ERR("init_lct_tp_proximity_data Failed!\n");
        goto err_init_lct_tp_proximity_data_failed;
    } else {
        NVT_LOG("init_lct_tp_proximity_data Succeeded!\n");
    }
//create longcheer procfs node
    ...
    if (ret < 0) {
        NVT_ERR("init_lct_tp_lockdown_info Failed!\n");
        goto err_init_lct_tp_lockdown_failed;
    } else {
        NVT_LOG("init_lct_tp_lockdown_info Succeeded!\n");
    }
#if LCT_TP_WORK_EN
...
#endif
    used_usb_mode = true;
    ts->event_wq = alloc_workqueue("nvt-event-queue", WQ_UNBOUND | WQ_HIGHPRI, 1);
    if (!ts->event_wq) {
    ...
    get_lockdown_info_by_lcd(); //read lockdown_info by lcd
    bTouchIsAwake = 1;
...
err_init_lct_tp_lockdown_failed:
    uninit_lct_tp_info();
...
uninit_lct_tp_proximity_data();
err_init_lct_tp_proximity_data_failed:
#if WAKEUP_GESTURE
...
NVT_LOG("Shutdown driver used_usb_mode = %d\n",used_usb_mode);
#if NVT_TOUCH_MP
    nvt_mp_proc_deinit();
#endif
#if NVT_TOUCH_EXT_PROC
...
#if WAKEUP_GESTURE
    if ((ts->is_gesture_mode) && (!lcd_nvt_ts_esd_resume)){
...
int lct_nvt_tp_gesture_callback(bool flag)
{
    ...
    if (flag) {
        ...
        set_lct_tp_gesture_status(true);
        NVT_LOG("enable gesture mode\n");
    } else {
        ts->is_gesture_mode = false;
        set_lct_tp_gesture_status(false);
...
static int32_t __init nvt_driver_init(void)
{
    ...
    if (lcm_name) {
        ...
        if (strstr(videolfb_tag->lcmname,"mdss_dsi_panel_c3f_43_02_0a_hd_video") != NULL) {
            strcpy(nvt_tp_fw_name, BOOT_UPDATE_FIRMWARE_NAME_XINLI);
            strcpy(nvt_tp_mp_name, MP_UPDATE_FIRMWARE_NAME_XINLI);
            NVT_LOG("[Vendor]xinli,[IC]nt36528a\n");
        } else if(strstr(videolfb_tag->lcmname,"mdss_dsi_panel_c3f_37_02_0b_hd_video") != NULL){
            strcpy(nvt_tp_fw_name, BOOT_UPDATE_FIRMWARE_NAME_TXD);
            strcpy(nvt_tp_mp_name, MP_UPDATE_FIRMWARE_NAME_TXD);
            NVT_LOG("[Vendor]TXD,[IC]nt36528a\n");
        } else {
