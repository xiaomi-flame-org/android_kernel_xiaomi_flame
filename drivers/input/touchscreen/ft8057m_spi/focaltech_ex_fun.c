...
static int32_t get_lockdown_info_by_lcd(void)
{
    ...
    if (lcm_name) {
        ...
        if (!videolfb_tag) {
            FTS_ERROR("Invalid lcm lockdown info\n");
            return -1;
        }
...
int lct_fts_get_tpfwver(const char *cmd)
{
    ...
    if ((fwver == 0xFF) || (fwver == 0x00)) {
        ...
        num_read_chars = snprintf(buf, 64, "[Vendor]HKC,[FW]0x%02x,[IC]ft8057m\n", fwver);
    }
    mutex_unlock(&input_dev->mutex);
...
int lct_create_procfs(struct fts_ts_data *ts_data)
{
    ...
    ret = init_lct_tp_info("[Vendor]HKC,[FW]0x%02x,[IC]ft8057m\n");
    if (ret) {
    ...
    if (ret < 0) {
        FTS_ERROR("init_lct_tp_lockdown_info Failed!\n");
        goto err_init_lct_tp_lockdown_failed;
    } else {
        FTS_INFO("init_lct_tp_lockdown_info Succeeded!\n");
    }
#endif
    ...
    if (ret < 0) {
        FTS_ERROR("init_lct_tp_proximity_data Failed!\n");
        goto err_init_lct_tp_proximity_data_failed;
    } else {
        FTS_ERROR("init_lct_tp_proximity_data Succeeded!\n");
    }
#endif

#if TP_INFO_TP_LOCKDOWN
    get_lockdown_info_by_lcd();
#endif
