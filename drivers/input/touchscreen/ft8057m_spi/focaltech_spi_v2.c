...
int current_usb_online;
int last_usb_online = -1;
#endif
...
#if LCT_TP_USB_PLUGIN
void fts_usb_online_work(struct work_struct *work)
{
    int ret;
...
int fts_read_reg_fw(u8 addr, u8 *value)
{
    return fts_read(&addr, 1, value, 1);
}
...
static int __init fts_ts_spi_init(void)
{
    ...
    if (lcm_name) {
        ...
        FTS_INFO("fts_driver_init read lcm name : %s", videolfb_tag->lcmname);

        if (strstr(videolfb_tag->lcmname, "mdss_dsi_panel_c3f_46_03_0c_hd_video") != NULL) {
            FTS_INFO("[Vendor]HKC,[IC]FT8057m\n");
        } else {
