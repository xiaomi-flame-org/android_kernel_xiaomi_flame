...
extern void fts_usb_online_work(struct work_struct *work);
static int fts_enter_proximity_mode(int mode)
{
    ...
    if (mode) {
        buf_value = 1;
        set_lct_tp_proximity_switch_status(true);
        tp_proximity_event = 1;
    } else {
        buf_value = 0;
        set_lct_tp_proximity_switch_status(false);
...
{
    int ret = 0;
    uint8_t psensor_status = 0;
    u8 buf_addr = 0;
    u8 buf_value = 0;
    buf_addr = FTS_REG_IDE_PARA_STATUS;
    if (fts_proximity_data.mode == DISABLE)
        return -EPERM;
    ret = fts_read_reg(FTS_REG_FACE_DEC_MODE_STATUS, &psensor_status);
    if (ret < 0) {
        FTS_ERROR("read proximity value failed,ret=%d", ret);
    } else {
        if (psensor_status == 0x01) {
#ifdef FACTORY_BUILD
            set_lct_tp_proximity_data(3);
#else
            set_lct_tp_proximity_data(1);
            FTS_DEBUG("psensor_status = %x", psensor_status);
#endif
            buf_value = 1;
            fts_write_reg(buf_addr, buf_value);
            fts_proximity_data.detect = PS_NEAR;
        } else if (psensor_status == 0x02) {
#ifdef FACTORY_BUILD
            set_lct_tp_proximity_data(3);
#else
            FTS_DEBUG("psensor_status = %x", psensor_status);
            set_lct_tp_proximity_data(2);
#endif
            buf_value = 1;
            fts_write_reg(buf_addr, buf_value);
            fts_proximity_data.detect = PS_NEAR;
        } else if (psensor_status == 0x03) {
            FTS_DEBUG("psensor_status = %x", psensor_status);
            set_lct_tp_proximity_data(3);
            buf_value = 1;
            fts_write_reg(buf_addr, buf_value);
            fts_proximity_data.detect = PS_NEAR;
        } else if (psensor_status == 0x00) {
            set_lct_tp_proximity_data(0);
            FTS_DEBUG("psensor_status = %x", psensor_status);
            buf_value = 0;
            fts_write_reg(buf_addr, buf_value);
            fts_proximity_data.detect = PS_FAR_AWAY;
            FTS_DEBUG("get proximity off event.\n");
        }
    }

    FTS_DEBUG("fts_proximity_data.detect is %d, psensor_status = %x", fts_proximity_data.detect, psensor_status);
    return -1;
}
...
int lct_fts_tp_gesture_callback(bool flag)
{
    ...
    FTS_INFO("lct_fts_tp_gesture_callback\n");
    ...
    if (flag) {
        FTS_INFO("lct_fts_tp_gesture_callback enable\n");
        ts_data->gesture_support = ENABLE;
    } else {
        FTS_INFO("lct_fts_tp_gesture_callback disable\n");
...
{
    if (ts_data->fwdbg_support && value)
    ...
    u8 id_h, u8 id_l, bool fw_valid)
{
    ...
    for (i = 0; i < ctype_entries; i++) {
        if (VALID == fw_valid) {
            if (((id_h == ctype[i].chip_idh) && (id_l == ctype[i].chip_idl)) || (!fts_match_cid(ts_data, ctype[i].type, id_h, id_l, 0)))
                break;
        } else {
...
static int fts_input_report_touch(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    ...
    for (i = 0; i < max_touch_num; i++) {
        ...
        events[i].x = ((touch_buf[FTS_TOUCH_OFF_E_XH + base] & 0x0F) << 12) + ((touch_buf[FTS_TOUCH_OFF_XL + base] & 0xFF) << 4) + ((touch_buf[FTS_TOUCH_OFF_PRE + base] >> 4) & 0x0F);
        events[i].y = ((touch_buf[FTS_TOUCH_OFF_ID_YH + base] & 0x0F) << 12) + ((touch_buf[FTS_TOUCH_OFF_YL + base] & 0xFF) << 4) + (touch_buf[FTS_TOUCH_OFF_PRE + base] & 0x0F);
...
#endif
#else
        events[i].x = ((touch_buf[FTS_TOUCH_OFF_E_XH + base] & 0x0F) << 8) + (touch_buf[FTS_TOUCH_OFF_XL + base] & 0xFF);
        events[i].y = ((touch_buf[FTS_TOUCH_OFF_ID_YH + base] & 0x0F) << 8) + (touch_buf[FTS_TOUCH_OFF_YL + base] & 0xFF);
...
static int fts_read_touchdata_spi(struct fts_ts_data *ts_data, u8 *buf)
{
    ...
    if (((0xEF == buf[1]) && (0xEF == buf[2]) && (0xEF == buf[3])) || ((ret < 0) && (0xEF == buf[0]))) {
...
#if FTS_PSENSOR_ENABLE
    if (fts_proximity_data.mode == ENABLE) {
        if ((fts_proximity_readdata(ts_data) != 0) && (ts_data->suspended == 1)) {
            return 1;
        }
    }
#endif
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    if (ts_data->gesture_support) {
        FTS_INFO("fts_ts_suspend fts_gesture_suspend\n");
...
static int fts_ts_resume(struct device *dev)
{
    ...
    FTS_FUNC_ENTER();
    if (!ts_data->suspended && !tp_proximity_event) {
    ...
    FTS_DEBUG("Notification type:%d, early_trigger:%d",
      ...
      case DRM_PANEL_EVENT_UNBLANK:
          if (notification->notif_data.early_trigger == 1) {
              FTS_INFO("resume pre commit\n");
          } else {
              FTS_INFO("resume function will be called\n");
      ...
      case DRM_PANEL_EVENT_BLANK_LP:
          if (notification->notif_data.early_trigger == 0) {
              FTS_INFO("DRM_PANEL_BLANK %s",
              notification->notif_type == DRM_PANEL_EVENT_BLANK_LP ? "POWER DOWN" : "LP");
              if (ts_data->suspended)
                  return;
              FTS_INFO("DRM_PANEL_BLANK %s, suspend function will be called",
              notification->notif_type == DRM_PANEL_EVENT_BLANK_LP ? "POWER DOWN" : "LP");
              flush_workqueue(ts_data->ts_workqueue);
                                             ...
                                             struct fts_ts_data *ts_data)
{
    ...
    void *cookie = NULL;
    /*
        rc = of_property_read_string(dp, "focaltech,touch-type", &touch_type);
       if (rc) {
            dev_warn(&fts_data->client->dev,"%s: No touch type\n", __func__);
            return;
       }
    ...
    */
    ...
    FTS_INFO("registered for panel notifications panel: 0x%x\n", active_panel);
...
#if defined(CONFIG_DRM_PANEL)
    if (active_panel) {
        FTS_DEBUG("enable ts_ts_register_for_panel_events");
