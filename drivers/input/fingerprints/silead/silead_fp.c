...
#define FP_INPUT_NAME "uinput-goodix"
...
static int silfp_probe(struct platform_device *spi)
{
    ...
    int    status = 0;
    //unsigned long minor;
    LOG_MSG_DEBUG(ERR_LOG, "silfp probe enter");
    LOG_MSG_DEBUG(INFO_LOG, "[%s] enter.\n", __func__);
...
int silfp_dev_init(void)
{
     ...
     */
    LOG_MSG_DEBUG(ERR_LOG, "silfp status num 1 %d.\n", status);
    status = register_chrdev(FP_DEV_MAJOR, "sil", &silfp_dev_fops);
    LOG_MSG_DEBUG(ERR_LOG, "silfp status num 2 %d.\n", status);
    if (status < 0) {
        return status;
    }
    LOG_MSG_DEBUG(ERR_LOG, "silfp register_chrdev succsues");
    ...
    LOG_MSG_DEBUG(ERR_LOG, "silfp class_create succsues");
    status = platform_driver_register(&silfp_driver);
    LOG_MSG_DEBUG(ERR_LOG, "silfp status num 3 %d.\n", status);
    if (status < 0) {
    ...
    LOG_MSG_DEBUG(ERR_LOG, "silfp driver succsues");
    return status;
}
