...
#endif
{
	...
	if (fpsensor == 1) {
		pr_info("%s, This is goodix fingerprint, fpsensor=%d\n", __func__, fpsensor);
	} else {
		pr_err("%s failed, This is not goodix fingerprint\n", __func__);
		return -1;
	}
...
//	gf_dev->irq = gf_irq_num(gf_dev);
	...
	/*proc_entry = proc_create(PROC_NAME, 0644, NULL, (const struct proc_ops *)(&proc_file_goodix_ops));
	if (NULL == proc_entry) {
